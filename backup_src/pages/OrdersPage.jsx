...
/* ─────────────────────────────────
   ORDER CARD
───────────────────────────────── */
function OrderCard({ order, onExpand, isExpanded, onCancel }) {
    ...
    >
      <div className="order-header" onClick={() => onExpand(order._id)}>
        ...
        <div className="order-right">
          ...
          <div className={`order-status-badge ${delivered ? 'badge-delivered' : cancelled ? 'badge-cancelled' : 'badge-active'}`}
            style={!(delivered || cancelled) ? { borderColor: order.color, color: order.color } : cancelled ? { borderColor: '#ef4444', color: '#ef4444' } : {}}>
            {delivered ? '✅ Delivered' : cancelled ? '❌ Cancelled' : `⚡ ${statusLabel}`}
          </div>
      ...
      {isExpanded && (
        <div className="order-detail">
          ...
          {!(delivered || cancelled) && (
            <button className="cancel-btn" onClick={() => onCancel(order._id)}>Cancel Order</button>
          )}
          {cancelled && (
            <div style={{color: '#ef4444', fontWeight: 600, marginTop: '20px', padding: '10px', background: 'rgba(239, 68, 68, 0.1)', borderRadius: '8px', textAlign: 'center'}}>
