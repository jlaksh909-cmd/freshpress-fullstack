...
function BookingModal({ onClose, preService, toast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast(err.message || 'Booking failed. Please sign in first.', 'error')
    } finally {
      setLoading(false)
    }
  }
...
function PriceCard({ plan, price, features, badge, highlight, onBook }) {
  const svc = null // booking without pre-selected service
  return (
    <div className={`price-card ${highlight ? 'price-highlight' : ''}`}>
      ...
      <button className="price-btn" onClick={() => onBook(svc)}>Get Started →</button>
    </div>
  )
}
    ...
    <div className="home-root">
      ...
      <section className="steps-section" id="howitworks">
        ...
        <div className="steps-cta">
          <button className="btn-primary" onClick={() => openBook()}>Start Now – Book a Pickup 🚀</button>
        </div>
      </section>
      ...
      <section className="stats-banner">
        ...
        <div className="stats-inner">
          <Stat n="50000" suffix="+" label="Garments Cleaned"/>
          <div className="stat-divider"/>
          <Stat n="15"    suffix="+" label="Cities Covered"/>
          <div className="stat-divider"/>
          <Stat n="4.9"   suffix="★" label="Average Rating"/>
          <div className="stat-divider"/>
          <Stat n="100"   suffix="%" label="Eco-Friendly"/>
        </div>
      </section>
      ...
      <section className="pricing-section" id="pricing">
        ...
        <div className="pricing-grid">
          <PriceCard plan="Basic Wash"    price="49"  features={['Wash & Fold','2-3 Day Delivery','Up to 5kg','Eco Detergents']}                                   onBook={openBook}/>
          <PriceCard plan="Premium Clean" price="149" badge="⭐ Most Popular" highlight features={['Dry Clean + Press','24h Turnaround','White-glove Handling','Stain Treatment','Free Pickup']} onBook={openBook}/>
          <PriceCard plan="Luxury Care"   price="349" features={['Designer Garments','Same-Day Service','Personal Garment Rep','Fragrance Finish','Priority Support']} onBook={openBook}/>
        </div>
      </section>
